...
// Brevo IP allowlists are often IPv4; prefer IPv4 so authorized 49.x.x.x matches.
...
export class EmailService {
  ...
  private isBrevoApiConfigured(): boolean {
    return Boolean(this.config.get<string>('email.brevoApiKey', '')?.trim());
  }
  ...
  /** Brevo shows this on SMTP tab — often `xxxx@smtp-brevo.com`, not your mailbox email. */
  private smtpLogin(): string {
    return (
      this.config.get<string>('email.brevoSmtpLogin', '')?.trim() ||
      this.config.get<string>('email.smtpUser', '')?.trim() ||
      ''
    );
  }
  ...
  private async getTransporter(): Promise<Transporter | null> {
    ...
    if (!this.isSmtpConfigured()) {
      if (!this.isBrevoApiConfigured()) {
        this.logger.warn(
          'Email not configured. Set BREVO_API_KEY (recommended) or SMTP_HOST + BREVO_SMTP_LOGIN + SMTP_PASS.',
        );
      }
      return null;
    }
    ...
    const host = this.config.get<string>('email.smtpHost', '');
    const port = this.config.get<number>('email.smtpPort', 587);
  ...
  private fromAddress(): string {
    return (
      this.config.get<string>('email.from', '')?.trim() ||
      this.smtpLogin() ||
  ...
  private appName(): string {
    return this.config.get<string>('email.appName', 'KNSR Mart') ?? 'KNSR Mart';
  }
  ...
  private async sendViaBrevoApi(params: MailPayload): Promise<SendMailResult> {
  ...
  private async sendViaSmtp(params: MailPayload): Promise<SendMailResult> {
  ...
  async sendMail(params: MailPayload): Promise<SendMailResult> {
    if (this.isBrevoApiConfigured()) {
      ...
      this.logger.warn(
        `Brevo API failed (${apiResult.reason}); trying SMTP fallback…`,
      );
    }
  ...
  /** @deprecated Use sendOnboardingCredentialsEmail */
